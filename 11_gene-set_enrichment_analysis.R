### STEP 9: Gene Set Enrichment Analysis of pathways across entire gene lists###

# Access DESeq2 results from CSV files
res_rna_hn <- read_csv(file = "results/rna_hn_results.csv")
res_rpf_hn <- read_csv(file = "results/rpf_hn_results.csv")
res_rna_gscnpc <- read_csv(file = "results/rna_gscnpc_results.csv")
res_rpf_gscnpc <- read_csv(file = "results/rpf_gscnpc_results.csv")

# Prepare geneLists for clusterProfiler
geneList_rna_hn <- res_rna_hn %>% dplyr::pull(log2FoldChange) # feature 1: numeric vector
names(geneList_rna_hn) <- res_rna_hn %>% dplyr::pull(entrez) %>% as.character() # feature 2: named vector
geneList_rna_hn <- sort(geneList_rna_hn, decreasing = TRUE) # feature 3: decreasing order

geneList_rna_gscnpc <- res_rna_gscnpc %>% dplyr::pull(log2FoldChange) # feature 1: numeric vector
names(geneList_rna_gscnpc) <- res_rna_gscnpc %>% dplyr::pull(entrez) %>% as.character() # feature 2: named vector
geneList_rna_gscnpc <- sort(geneList_rna_gscnpc, decreasing = TRUE) # feature 3: decreasing order
...
y_gscnpc <- gsePathway(geneList_rna_gscnpc,
                       pvalueCutoff = 0.2,
                       pAdjustMethod = "BH",
                       verbose = TRUE)
dotplot(y_gscnpc,
